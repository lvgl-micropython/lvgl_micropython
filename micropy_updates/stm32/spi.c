...
static mp_lcd_spi_bus_obj_t *sending_device;

static void DMATransmitCplt(DMA_HandleTypeDef *hdma) {
    HAL_StatusTypeDef status = HAL_SPI_UnRegisterCallback(hdma, HAL_SPI_TX_COMPLETE_CB_ID);

    if (status != HAL_OK) {
        mp_hal_raise(status);
    }

    spi_t *spi_obj = (spi_t *)sending_device->spi_device->spi_bus->user_data;
    dma_deinit(spi_obj->tx_dma_descr);

    sending_device.dma_callback(sending_device);
    sending_device = NULL;
}

void spi_transfer(mp_obj_base_t *self_in, size_t len, const uint8_t *src, uint8_t *dest, uint32_t timeout) {
    ...
    if (self_in->base.type == &mp_machine_hw_spi_device_type) {
        ...
        while (sending_device != NULL) { }

        sending_device = (mp_lcd_spi_bus_obj_t *)self_in;
        self = sending_device->spi_device;
    } else {
        return;
    }

    const spi_t *spi_obj = (const spi_t *)self->spi_bus->user_data;
    ...
    if (dest == NULL) {
        // send only
        if (
            len == 1 && (sending_device == NULL || !sending_device->use_dma) ||
            (query_irq() == IRQ_STATE_DISABLED && (sending_device == NULL || !sending_device->use_dma)) {
            status = HAL_SPI_Transmit(spi_obj->spi, (uint8_t *)src, len, timeout);
        } else {
            if (sending_device != NULL && sending_device->use_dma) {
                status = HAL_SPI_RegisterCallback(spi_obj->spi, HAL_SPI_TX_COMPLETE_CB_ID, &DMATransmitCplt);
            ...
            dma_init(&tx_dma, spi_obj->tx_dma_descr, DMA_MEMORY_TO_PERIPH, spi_obj->spi);
            spi_obj->spi->hdmatx = &tx_dma;
            spi_obj->spi->hdmarx = NULL;
            MP_HAL_CLEAN_DCACHE(src, len);
            uint32_t t_start = HAL_GetTick();
            do {
                uint32_t l = MIN(len, 65535);
                status = HAL_SPI_Transmit_DMA(spi_obj->spi, (uint8_t *)src, l);
                if (status != HAL_OK) {
                    break;
                }
                if (sending_device == NULL) {
                    status = spi_wait_dma_finished(spi_obj, t_start, timeout);
                    if (status != HAL_OK) {
                        break;
                    }
                }
        ...
        if (len == 1 || query_irq() == IRQ_STATE_DISABLED) {
            status = HAL_SPI_Receive(spi_obj->spi, dest, len, timeout);
        } else {
            DMA_HandleTypeDef tx_dma, rx_dma;
            if (spi_obj->spi->Init.Mode == SPI_MODE_MASTER) {
                // in master mode the HAL actually does a TransmitReceive call
                dma_init(&tx_dma, spi_obj->tx_dma_descr, DMA_MEMORY_TO_PERIPH, spi_obj->spi);
                spi_obj->spi->hdmatx = &tx_dma;
            } else {
                spi_obj->spi->hdmatx = NULL;
            }
            dma_init(&rx_dma, spi_obj->rx_dma_descr, DMA_PERIPH_TO_MEMORY, spi_obj->spi);
            spi_obj->spi->hdmarx = &rx_dma;
            dma_protect_rx_region(dest, len);
            uint32_t t_start = HAL_GetTick();
            do {
                uint32_t l = MIN(len, 65535);
                status = HAL_SPI_Receive_DMA(spi_obj->spi, dest, l);
                if (status != HAL_OK) {
                    break;
                }
                status = spi_wait_dma_finished(spi_obj, t_start, timeout);
        ...
        // send and receive
        if (len == 1 || query_irq() == IRQ_STATE_DISABLED) {
            status = HAL_SPI_TransmitReceive(spi_obj->spi, (uint8_t *)src, dest, len, timeout);
        } else {
            DMA_HandleTypeDef tx_dma, rx_dma;
            dma_init(&tx_dma, spi_obj->tx_dma_descr, DMA_MEMORY_TO_PERIPH, spi_obj->spi);
            spi_obj->spi->hdmatx = &tx_dma;
            ...
            do {
                uint32_t l = MIN(len, 65535);
                status = HAL_SPI_TransmitReceive_DMA(spi_obj->spi, (uint8_t *)src, dest, l);
                if (status != HAL_OK) {
                    break;
                }
                status = spi_wait_dma_finished(spi_obj, t_start, timeout);
...
static void spi_proto_transfer(mp_obj_base_t *self_in, size_t len, const uint8_t *src, uint8_t *dest) {
