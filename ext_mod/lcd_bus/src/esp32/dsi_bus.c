...
#include "soc/soc_caps.h"
    ...
    static mp_obj_t mp_lcd_dsi_bus_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args)
    {
        ...
        self->panel_io_config = malloc(sizeof(esp_lcd_dbi_io_config_t));
        esp_lcd_dbi_io_config_t *panel_io_config = self->panel_io_config;

        self->bus_config = malloc(sizeof(esp_lcd_dsi_bus_config_t));
        esp_lcd_dsi_bus_config_t *bus_config = self->bus_config;

        self->panel_config = malloc(sizeof(esp_lcd_dpi_panel_config_t));
        esp_lcd_dpi_panel_config_t *panel_config = self->panel_config;

        bus_config->bus_id = (int)args[ARG_bus_id].u_int;
        bus_config->num_data_lanes = (uint8_t)args[ARG_data_lanes].u_int;
        bus_config->lane_bit_rate_mbps = (uint32_t)args[ARG_freq].u_int;
        bus_config->phy_clk_src = MIPI_DSI_PHY_CLK_SRC_DEFAULT;

        panel_io_config->virtual_channel = (uint8_t)args[ARG_virtual_channel].u_int;

        panel_config->virtual_channel = (uint8_t)args[ARG_virtual_channel].u_int;
        panel_config->dpi_clk_src = MIPI_DSI_DPI_CLK_SRC_DEFAULT;

        panel_config->dpi_clock_freq_mhz = (uint32_t)args[ARG_freq].u_int;

        panel_config->video_timing.hsync_back_porch = (uint32_t)args[ARG_hsync_back_porch].u_int;
        panel_config->video_timing.hsync_pulse_width = (uint32_t)args[ARG_hsync_pulse_width].u_int;
        panel_config->video_timing.hsync_front_porch = (uint32_t)args[ARG_hsync_front_porch].u_int;
        panel_config->video_timing.vsync_back_porch = (uint32_t)args[ARG_vsync_back_porch].u_int;
        panel_config->video_timing.vsync_pulse_width = (uint32_t)args[ARG_vsync_pulse_width].u_int;
        panel_config->video_timing.vsync_front_porch = (uint32_t)args[ARG_vsync_front_porch].u_int;
        ...
        LCD_DEBUG_PRINT("hsync_pulse_width=%d\n", panel_config->video_timing.hsync_pulse_width)
