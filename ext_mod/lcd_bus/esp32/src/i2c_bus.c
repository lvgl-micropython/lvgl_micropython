                                        ...
                                        size_t n_kw, const mp_obj_t *all_args)
{
    ...
    self->panel_io_config.scl_speed_hz = (uint32_t)args[ARG_freq].u_int
    ...
    if (self->panel_io_handle.panel_io != NULL) {
        ...
        if (ret != 0) {
            mp_raise_msg_varg(&mp_type_ValueError,
                              MP_ERROR_TEXT("%d(esp_lcd_panel_io_del)"), ret);
            return ret;
        }
                      ...
                      uint8_t param_bits)
{
    ...
    ret = esp_lcd_new_panel_io_i2c_v2((i2c_master_bus_handle_t)self->i2c_device->i2c_bus->user_data , &self->panel_io_config,
                                   &self->panel_io_handle.panel_io);

    if (ret != 0) {
        mp_raise_msg_varg(&mp_type_ValueError,
                          MP_ERROR_TEXT("%d(esp_lcd_new_panel_io_i2c_v2)"), ret);
        return ret;
    }
