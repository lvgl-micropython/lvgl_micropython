...
static mp_lcd_err_t spi_rotation_init_func(void *self_in)
{
    ...
    if (init_err->code != LCD_OK) {
        init_err->msg = MP_ERROR_TEXT("%d(esp_lcd_new_panel_io_spi)");
...
static mp_obj_t mp_lcd_spi_bus_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args)
{
    ...
    self->panel_io_config = (esp_lcd_panel_io_spi_config_t *)malloc(sizeof(esp_lcd_panel_io_spi_config_t));
    ...
    if (self->panel_io_config->flags.sio_mode == 1) self->lane_count = 2;
    else if (self->panel_io_config->flags.quad_mode == 1) self->lane_count = 4;
    else if (self->panel_io_config->flags.octal_mode == 1) self->lane_count = 8;
    else self->lane_count = 1;
