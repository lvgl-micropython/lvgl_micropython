#include "soc/soc_caps.h"
    ...
    mp_lcd_err_t rgb_init(mp_obj_t obj, uint16_t width, uint16_t height, uint8_t bpp, uint32_t buffer_size, bool rgb565_byte_swap, uint8_t cmd_bits, uint8_t param_bits, bool sw_rotate);
                                    ...
                                    const esp_lcd_rgb_panel_event_data_t *edata, void *user_ctx)
    {
        ...
        rotation_buffer_t *buf = &self->rotation->buf;
        rotation_task_t *task = &self->rotation->task;

        if (curr_buf != buf->active && !bus_event_isset_from_isr(&task->swap_bufs)) {
            void *idle_fb = buf->idle;
            buf->idle = buf->active;
            buf->active = idle_fb;
            bus_event_set_from_isr(&task->swap_bufs);
        }
    ...
    static mp_lcd_err_t rgb_rotation_init_func(void *self_in)
    {
        mp_lcd_rgb_bus_obj_t *self = (mp_lcd_rgb_bus_obj_t *)self_in;

        rotation_init_err_t *init_err = &self->rotation->init_err;

        init_err->code = esp_lcd_new_rgb_panel(self->panel_io_config, &self->panel_handle);
        if (init_err->code != 0) {
            init_err->msg = MP_ERROR_TEXT("%d(esp_lcd_new_rgb_panel)");
            return init_err->code;
        }

        esp_lcd_rgb_panel_event_callbacks_t callbacks = { .on_vsync = rgb_bus_trans_done_cb };
        init_err->code = esp_lcd_rgb_panel_register_event_callbacks(self->panel_handle, &callbacks, self);
        if (init_err->code != 0) {
            init_err->msg = MP_ERROR_TEXT("%d(esp_lcd_rgb_panel_register_event_callbacks)");
            return init_err->code;
        }

        init_err->code = esp_lcd_panel_reset(self->panel_handle);
        if (init_err->code != 0) {
            init_err->msg = MP_ERROR_TEXT("%d(esp_lcd_panel_reset)");
            return init_err->code;
        }

        init_err->code = esp_lcd_panel_init(self->panel_handle);
        if (init_err->code != 0) {
            init_err->msg = MP_ERROR_TEXT("%d(esp_lcd_panel_init)");
            return init_err->code;
        }
    ...
    static void rgb_rotation_last_update_func(void *self_in, void *idle_buf)
    {
        mp_lcd_rgb_bus_obj_t *self = (mp_lcd_rgb_bus_obj_t *)self_in;
        ...
        mp_lcd_err_t ret = esp_lcd_panel_draw_bitmap(
            ...
            0,
            0,
            self->rotation->data.width - 1,
            self->rotation->data.height - 1,
            idle_buf
        );
        ...
        if (ret != 0) {
            ...
            bus_event_clear(&self->rotation->task.swap_bufs);
            bus_event_wait(&self->rotation->task.swap_bufs);
            memcpy(self->rotation->buf.idle, self->rotation->buf.active,
                self->rotation->data.width * self->rotation->data.height * self->rotation->data.bytes_per_pixel);
        }
    }
    ...
    mp_obj_t mp_lcd_rgb_bus_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args)
    {
        ...
        esp_lcd_rgb_timing_t *bus_config = &self->panel_io_config->timings;
    ...
    mp_lcd_err_t rgb_del(mp_obj_t obj)
    {
        ...
        bus_event_set(&self->rotation->task.exit);
        bus_lock_release(&self->rotation->task.lock);
        bus_lock_release(&self->rotation->task.tx_color_lock);

        mp_lcd_err_t ret = esp_lcd_panel_del(self->panel_handle);

        bus_lock_delete(&self->rotation->task.lock);
        bus_lock_delete(&self->rotation->task.tx_color_lock);

        bus_event_clear(&self->rotation->task.swap_bufs);
        bus_event_clear(&self->rotation->task.swap_bufs);
        bus_event_clear(&self->rotation->task.exit);
    ...
    mp_lcd_err_t rgb_init(mp_obj_t obj, uint16_t width, uint16_t height, uint8_t bpp, uint32_t buffer_size, bool rgb565_byte_swap, uint8_t cmd_bits, uint8_t param_bits, bool sw_rotate)
    {
        ...
        self->rotation->init_func = &rgb_rotation_init_func;
        self->rotation->last_update_func = &rgb_rotation_last_update_func;
