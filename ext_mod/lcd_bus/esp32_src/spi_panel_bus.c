...
/*
 * This driver is a modified version of the spi driver in the esp_lcd
 * component of the ESP-IDF. I modified this driver so it will use polling
 * when double buffering or DMA memory is not being used and sending the frame
 * buffer data. By doing this it eliminates the very large number of interrupts
 * that occur. This increases the speed in which the data is able to transfer
 * by approx 50%
 */
...
static const char *TAG = "lcdbus.io.spi";

static esp_err_t lcdbus_io_spi_rx_param(esp_lcd_panel_io_t *io, int lcd_cmd, void *param, size_t param_size);
static esp_err_t lcdbus_io_spi_tx_param(esp_lcd_panel_io_t *io, int lcd_cmd, const void *param, size_t param_size);
static esp_err_t lcdbus_io_spi_tx_color(esp_lcd_panel_io_t *io, int lcd_cmd, const void *color, size_t color_size);
static esp_err_t lcdbus_io_spi_del(esp_lcd_panel_io_t *io);
static void lcdbus_spi_pre_trans_cb(spi_transaction_t *trans);
static void lcdbus_spi_post_trans_color_cb(spi_transaction_t *trans);
static esp_err_t lcdbus_io_spi_register_event_callbacks(esp_lcd_panel_io_handle_t io, const esp_lcd_panel_io_callbacks_t *cbs, void *user_ctx);
...
typedef struct {
    ...
    lcdbus_spi_trans_descriptor_t trans_pool[]; // Transaction pool
} lcdbus_io_spi_t;
...
esp_err_t lcdbus_new_panel_io_spi(esp_lcd_spi_bus_handle_t bus, const esp_lcd_panel_io_spi_config_t *io_config, esp_lcd_panel_io_handle_t *ret_io, uint8_t double_buffer)
{
    ...
    lcdbus_io_spi_t *spi_panel_io = NULL;
    ESP_GOTO_ON_FALSE(bus && io_config && ret_io, ESP_ERR_INVALID_ARG, err, TAG, "invalid argument");
    spi_panel_io = calloc(1, sizeof(lcdbus_io_spi_t) + sizeof(lcdbus_spi_trans_descriptor_t) * io_config->trans_queue_depth);
    ...
    spi_device_interface_config_t devcfg = {
        ...
        .pre_cb = lcdbus_spi_pre_trans_cb, // pre-transaction callback, mainly control DC gpio level
        .post_cb = lcdbus_spi_post_trans_color_cb, // post-transaction, where we invoke user registered "on_color_trans_done()"
    };
    ...
    spi_panel_io->double_buffer = double_buffer;
    ...
    spi_panel_io->base.rx_param = lcdbus_io_spi_rx_param;
    spi_panel_io->base.tx_param = lcdbus_io_spi_tx_param;
    spi_panel_io->base.tx_color = lcdbus_io_spi_tx_color;
    spi_panel_io->base.del = lcdbus_io_spi_del;
    spi_panel_io->base.register_event_callbacks = lcdbus_io_spi_register_event_callbacks;
...
static esp_err_t lcdbus_io_spi_del(esp_lcd_panel_io_t *io)
{
    ...
    lcdbus_io_spi_t *spi_panel_io = __containerof(io, lcdbus_io_spi_t, base);
...
static void lcdbus_spi_prepare_cmd_buffer(lcdbus_io_spi_t *panel_io, const void *cmd)
{
...
static void lcdbus_spi_prepare_param_buffer(lcdbus_io_spi_t *panel_io, const void *param, size_t param_size)
{
...
static esp_err_t lcdbus_io_spi_tx_param(esp_lcd_panel_io_t *io, int lcd_cmd, const void *param, size_t param_size)
{
    esp_err_t ret = ESP_OK;
    spi_transaction_t *spi_trans = NULL;
    lcdbus_spi_trans_descriptor_t *lcd_trans = NULL;
    lcdbus_io_spi_t *spi_panel_io = __containerof(io, lcdbus_io_spi_t, base);
    ...
    memset(lcd_trans, 0, sizeof(lcdbus_spi_trans_descriptor_t));
    ...
    if (send_cmd) {
        lcdbus_spi_prepare_cmd_buffer(spi_panel_io, &lcd_cmd);
    ...
    if (param && param_size) {
        lcdbus_spi_prepare_param_buffer(spi_panel_io, param, param_size);
...
static esp_err_t lcdbus_io_spi_tx_color(esp_lcd_panel_io_t *io, int lcd_cmd, const void *color, size_t color_size)
{
    ...
    if (!spi_panel_io->double_buffer) {
        if (spi_panel_io->dc_gpio_num >= 0) {
            gpio_set_level(spi_panel_io->dc_gpio_num, spi_panel_io->flags.dc_data_level);
        }
    }
    ...
    do {
        ...
        if (spi_panel_io->num_trans_inflight < spi_panel_io->queue_size) {
            ...
            ret = spi_device_get_trans_result(spi_panel_io->spi_dev, &spi_trans, portMAX_DELAY);
            ESP_GOTO_ON_ERROR(ret, err, TAG, "recycle spi transactions failed");
            lcd_trans = __containerof(spi_trans, lcdbus_spi_trans_descriptor_t, base);
            spi_panel_io->num_trans_inflight--;
        }
        memset(lcd_trans, 0, sizeof(lcdbus_spi_trans_descriptor_t));
...
static void lcdbus_spi_pre_trans_cb(spi_transaction_t *trans)
{
    lcdbus_io_spi_t *spi_panel_io = trans->user;
    lcdbus_spi_trans_descriptor_t *lcd_trans = __containerof(trans, lcdbus_spi_trans_descriptor_t, base);
