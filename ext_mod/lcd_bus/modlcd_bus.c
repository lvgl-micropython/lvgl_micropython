...
{
    enum { ARG_self, ARG_cmd, ARG_data, ARG_x_start, ARG_y_start, ARG_x_end, ARG_y_end, ARG_rotation, ARG_last_flush };
    static const mp_arg_t allowed_args[] = {
        { MP_QSTR_self,       MP_ARG_OBJ  | MP_ARG_REQUIRED, { .u_obj  = mp_const_none } },
        { MP_QSTR_cmd,        MP_ARG_INT  | MP_ARG_REQUIRED, { .u_int  = -1            } },
        ...
        { MP_QSTR_last_flush, MP_ARG_BOOL | MP_ARG_KW_ONLY,  { .u_bool = true          } },
    };
    ...
    mp_lcd_err_t ret = lcd_panel_io_tx_color(
        ...
        (int)args[ARG_rotation].u_int,
        (bool)args[ARG_last_flush].u_bool
    );
...
MP_DEFINE_CONST_FUN_OBJ_KW(mp_lcd_bus_tx_color_obj, 9, mp_lcd_bus_tx_color);
