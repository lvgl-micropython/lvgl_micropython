...
#define MP_DEFINE_CONST_LV_FUN_OBJ_VAR(obj_name, n_args, mp_fun, lv_fun) \
    const mp_lv_obj_fun_builtin_var_t obj_name = { { &mp_lv_type_fun_builtin_var }, n_args, mp_fun, lv_fun }

#define MP_DEFINE_CONST_LV_FUN_OBJ_STATIC_VAR(obj_name, n_args, mp_fun, lv_fun) \
    const mp_lv_obj_fun_builtin_var_t obj_name = { { &mp_lv_type_fun_builtin_static_var }, n_args, mp_fun, lv_fun }
...
static mp_obj_t cast(mp_obj_t mp_obj, const mp_obj_type_t *mp_type)
{
    ...
    if (res == NULL) {
        nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_SyntaxError, MP_ERROR_TEXT("Can't convert %s to %s!"), mp_obj_get_type_str(mp_obj), qstr_str(mp_type->name)));
    }
...
static inline LV_OBJ_T *mp_to_lv(mp_obj_t mp_obj)
{
    ...
    if (mp_lv_obj->lv_obj == NULL) {
        nlr_raise(mp_obj_new_exception_msg(&mp_type_LvReferenceError, MP_ERROR_TEXT("Referenced object was deleted!")));
    }
...
static inline LV_OBJ_T *mp_get_callbacks(mp_obj_t mp_obj)
{
    ...
    if (mp_lv_obj == NULL) {
        nlr_raise(mp_obj_new_exception_msg(&mp_type_SyntaxError, MP_ERROR_TEXT("'user_data' argument must be either a dict or None!")));
    }
...
static mp_obj_t make_new(const mp_lv_obj_fun_builtin_var_t *lv_obj_var, const mp_obj_type_t *type,
...
static mp_lv_struct_t *mp_to_lv_struct(mp_obj_t mp_obj)
{
    ...
    if (!MP_OBJ_IS_OBJ(native_obj) ||
        MP_OBJ_TYPE_GET_SLOT_OR_NULL(mp_obj_get_type(native_obj), make_new) != &make_new_lv_struct) {

        nlr_raise(mp_obj_new_exception_msg(&mp_type_SyntaxError, MP_ERROR_TEXT("Expected Struct object!")));
    }
...
static mp_obj_t make_new_lv_struct(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{

    if ((!MP_OBJ_IS_TYPE(type, &mp_type_type)) || MP_OBJ_TYPE_GET_SLOT_OR_NULL(type, make_new) != &make_new_lv_struct) {
        nlr_raise(mp_obj_new_exception_msg(&mp_type_SyntaxError, MP_ERROR_TEXT("Argument is not a struct type!")));
    }
...
static mp_obj_t lv_struct_subscr(mp_obj_t self_in, mp_obj_t index, mp_obj_t value)
{
    ...
    if (!mp_obj_is_int(index)) {
        nlr_raise(mp_obj_new_exception_msg(&mp_type_SyntaxError, MP_ERROR_TEXT("Subscript index must be an integer!")));
    }
...
//        return (void*)mp_obj_get_int(self_in);
    ...
    if (!mp_get_buffer(self_in, &buffer_info, MP_BUFFER_READ)) {
        ...
        if (MP_OBJ_IS_TYPE(self_in, &mp_type_dict)) {
            ...
            nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_SyntaxError, MP_ERROR_TEXT("Cannot convert '%s' to pointer!"), mp_obj_get_type_str(self_in)));
        }
    }
    ...
    if (MP_OBJ_IS_STR_OR_BYTES(self_in) ||
        ...
        if (buffer_info.len != sizeof(result) ||
            ...
            nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_SyntaxError, MP_ERROR_TEXT("Cannot convert %s to pointer! (buffer does not represent a pointer)"), mp_obj_get_type_str(self_in)));
        }
...
static mp_obj_t mp_blob_cast(size_t argc, const mp_obj_t *argv)
{
    ...
    if (!MP_OBJ_IS_TYPE(type, &mp_type_type)) {
        nlr_raise(mp_obj_new_exception_msg(&mp_type_SyntaxError, MP_ERROR_TEXT("Cast argument must be a type!")));
    }
...
static mp_obj_t mp_lv_dereference(size_t argc, const mp_obj_t *argv)
{
    ...
    mp_obj_array_t *view = MP_OBJ_TO_PTR(mp_obj_new_memoryview(BYTEARRAY_TYPECODE, size, self->data));
                              ...
                              void *lv_callback, qstr func_name, void *user_data)
{
    ...
    if (lv_fun == lv_callback) {
        ...
        if (callbacks) return mp_obj_dict_get(callbacks, MP_OBJ_NEW_QSTR(func_name));
    }
...
static void mp_lv_array_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind)
{
...
static mp_obj_t lv_array_subscr(mp_obj_t self_in, mp_obj_t index, mp_obj_t value)
{
    ...
    if (value == MP_OBJ_NULL) memset(element_addr, 0, element_size);
        ...
        if (!mp_obj_is_int(value)) {
            nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_SyntaxError, MP_ERROR_TEXT("Value '%s' must be an integer!"), mp_obj_get_type_str(value)));
        }
...
GENMPY_UNUSED static void *mp_array_to_ptr(mp_obj_t *mp_arr, size_t element_size, GENMPY_UNUSED bool is_signed)
{
    ...
    while ((item = mp_iternext(iter)) != MP_OBJ_STOP_ITERATION) {
        ...
        if (!mp_obj_is_int(item)) {
            nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_SyntaxError, MP_ERROR_TEXT("Value '%s' must be an integer!"), mp_obj_get_type_str(item)));
        }
